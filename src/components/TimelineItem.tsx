...
export function TimelineItem({ event, isLeft }: TimelineItemProps) {
  ...
  return (
    ...
    {isModalOpen && (
      ...
      >
        <div 
          className="max-w-4xl w-full bg-white rounded-xl overflow-hidden flex flex-col md:flex-row shadow-2xl relative"
        ...
        >
          ...
          {event.image_url && (
            <div className="md:w-3/5 bg-black flex items-center justify-center">
              <img src={event.image_url} alt={event.title} className="max-h-[80vh] w-auto object-contain" />
            </div>
          )}
          <div className={`p-8 flex flex-col ${event.image_url ? 'md:w-2/5' : 'w-full'} max-h-[80vh] overflow-y-auto`}>
            <h2 className="text-3xl font-bold mb-2 text-gray-800">{event.title}</h2>
            <p className="text-cyan-600 font-mono mb-6 font-semibold">{event.event_date}</p>
